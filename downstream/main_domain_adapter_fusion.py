...
from models.BERT_DST_Picklist_adapter_fusion import *
from models.dual_encoder_ranking_adapter_fusion import *
...
if args["do_train"]:
    ...
    for run in range(args["nb_runs"]):
        ...
        #model.utterance_encoder.save_all_adapter_fusions(args["save_adapter_path"])
        model.load_state_dict(torch.load(output_model_file))
        print(args['save_adapter_path']+'/NSP_hotel,NSP_attraction,NSP_taxi')
        #model.utterance_encoder.load_adapter_fusion(args['save_adapter_path']+'/NSP_hotel,NSP_attraction,NSP_taxi', set_active=True)
        
        logging.info("Model: {}".format(model))
        for (n, p) in model.named_parameters():
            logging.info("{}:{}".format(n, p.requires_grad))
        ...
        if args["not_save_model"]:
            ...
            if torch.cuda.is_available(): 
                ...
                model.utterance_encoder.load_state_dict(torch.load(output_model_file, lambda storage, loc: storage))
                #model.utterance_encoder.load_adapter_fusion(args['save_adapter_path']+'/NSP_hotel,NSP_attraction,NSP_taxi', set_active=True)
